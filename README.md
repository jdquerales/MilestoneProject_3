...
Go to root directory of workspace (I used Gitpod here). 

1. Create a requirements file that will be used by Heroku. It contains all dependences and packagues
needed for suceesfully run the application. In the command line interface (cli)  it can be done by 
running the following command: `pip3 freeze --local > requirements.txt`. If new libraries are installed
while developing the project, the **requirements.txt** file should be updated.
2. Create a Procfile (It will tell Heroku how to run the application).
 In the cli it can be done by running the following command: `echo web: python app.py > Procfile` 
 "Note: make sure you have an empty new line after app.py". Description of the Procfile format can be found at (https://devcenter.heroku.com/articles/procfile).
3. As I had signed up for Heroku before (If you do not an account just create it!), I Logged in to Heroku,
...
6. From the inputs below, make sure your github user is selected, and then enter the name for your repo.
   Click "search". When it finds the repo, click the "connect" button. 
7. Scroll back up and click "settings". Scroll down and click "Reveal config vars". 
   Set up your variables for IP, PORT, SECRET_KEY, MONGO_URI and MONGODB_NAME.
8. Scroll back up and click "Deploy". Scroll down and click "Enable automatic deployment". 
9. Finally click "Deploy branch". Heroku will now start building the app. 
   When the build is complete, click "Open app" to open it.
...
*Note: You will have to install all the dependencies from [requirements](https://github.com/jdquerales/MilestoneProject_3/blob/master/requirements.txt) for the app to work.
